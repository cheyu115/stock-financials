...
- 修正兩點：查詢和搜尋欄高度不一致，且佔用太多橫向空間，置中，限制寬度為網頁的一半，結果卡片也要做一樣的對齊
- html上搜尋和查詢的form仍然佔滿整個螢幕寬度，修改為50%的寬度並置中
-  當結果卡片產生後，置中的輸入表單會被往上推動。上調輸入表單或著讓下移結果卡片，我要讓這個表單固定在畫面上，不管有沒有結果卡片存在。 